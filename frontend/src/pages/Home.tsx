...
import { extractPdfText, predictPdf, predictText, predictUrl } from "../lib/api";
...
export function Home() {
  ...
  async function onAnalyze() {
    ...
    try {
      ...
      } else if (tab === "file" && isPdfFile && fileObj) {
        const data = await predictPdf(fileObj, model);
        if (data.error) setErr(data.error);
        else {
          setResult(data);
          pushHistory("text", `${fileObj.name} (PDF)`, data);
        }
      } else {
        if (!text.trim()) {
  ...
  async function loadFile(f: File) {
    ...
    if (isPdf) {
      ...
      try {
        const data = await extractPdfText(f);
        if (data.error) {
          setErr(data.error);
          setText("");
          return;
        }
        setText((data.full_text || "").slice(0, 15000));
      } catch (e) {
        setErr(e instanceof Error ? e.message : "Failed to extract text from PDF.");
      } finally {
    ...
    try {
      ...
      if (!sid) {
        const out = await createShareLink({
          source_type: tab,
          input: tab === "url" ? url : text.slice(0, 2000),
          result,
        });
        if (out.error || !out.share_id) {
          setErr(out.error || "Could not create share link.");
          return;
        }
        sid = out.share_id;
      }
      const link = `${window.location.origin}/result/${sid}`;
      await navigator.clipboard.writeText(link);
    } catch (e) {
      setErr(e instanceof Error ? e.message : "Failed to copy share link.");
    } finally {
      setShareBusy(false);
    }
  }
    ...
    <div className="space-y-10 sm:space-y-12">
      ...
      <div className="grid items-start gap-6 lg:grid-cols-5 lg:gap-8">
        <div className="space-y-5 lg:col-span-2">
          ...
          {tab === "file" && (
            <div
              className="glass-panel border-2 border-dashed border-slate-200 p-8 text-center"
              onDragOver={(e) => e.preventDefault()}
              onDrop={(e) => void onDrop(e)}
            >
              <UploadCloud className="mx-auto mb-3 h-7 w-7 text-sky-600" />
              <input
                type="file"
              ...
              {pdfPreviewLoading && (
                <p className="mt-2 text-2xs text-slate-500">Extracting PDF text preview…</p>
              )}
              {fileName && text && (
                <div className="mt-4 rounded-xl border border-slate-200 bg-white p-3 text-left">
                  <p className="mb-1 text-2xs font-bold uppercase tracking-wider text-slate-500">
                    Extracted preview
                  </p>
                  <p className="max-h-32 overflow-y-auto text-xs leading-relaxed text-slate-700">
                    {text.slice(0, 600)}
                    {text.length > 600 ? "..." : ""}
                  </p>
                </div>
              ...
              <p className="mt-2 text-2xs text-slate-500">
                Drag and drop or browse: .txt, .md, .text, .pdf
              </p>
            </div>
          )}
        ...
        >
          ...
          {result && !result.error && (
            <div className="relative space-y-6">
              ...
              <SentenceClaimsPanel claims={result.sentence_claims} />

              <div className="rounded-2xl border border-slate-200 bg-white/80 p-4 sm:p-5">
                <p className="text-2xs font-bold uppercase tracking-widest text-slate-500">
                  External fact checks
                </p>
                {!!result.fact_checks?.length ? (
                  <div className="mt-3 space-y-3">
                    {result.fact_checks.map((fc, i) => (
                      <div key={`${fc.url}-${i}`} className="rounded-xl border border-slate-200 p-3">
                        <p className="text-xs font-semibold text-slate-800">{fc.source_name}</p>
                        <p className="mt-1 text-xs text-slate-600">
                          Rating: <span className="font-medium">{fc.rating || "Unrated"}</span>
                        </p>
                        <p className="mt-1 text-2xs text-slate-500 line-clamp-2">{fc.matched_claim}</p>
                        {fc.url ? (
                          <a
                            href={fc.url}
                            target="_blank"
                  ...
                  <p className="mt-2 text-xs text-slate-500">
                    No matching external fact-check claims found for this input.
                  </p>
                )}
              </div>
              ...
              {result.model && (
                <div className="flex flex-wrap items-center justify-between gap-3 border-t border-slate-200 pt-4">
                  <p className="text-2xs text-slate-500">
                    Engine: {result.model}
                    {result.language_model ? ` • Language route: ${result.language_model}` : ""}
                  </p>
                  <button
                    type="button"
                    onClick={() => void onCopyShareLink()}
                    disabled={shareBusy}
                    className="rounded-lg border border-sky-200 px-3 py-1.5 text-2xs font-semibold text-sky-700 hover:bg-sky-50 disabled:opacity-60"
                  >
                    {shareBusy ? "Preparing..." : "Copy Link"}
                  </button>
                </div>
              )}
            </div>
